...
parent_dir = os.path.join(script_dir, "xml")
...
def clean_name(badname):
    ...
    name = "".join(
        [(i if (64 < ord(i) < 91 or 96 < ord(i) < 123) else "")
         for i in badname]
    )
    return name.lower()


async def get_product_info(session, product_id):
    """Get product information from Microsoft API"""
    ...
    async with session.get(details_api) as response:
        ...
        if not response_json.get("Payload", None):
            raise Exception("No data found for product ID")

        return response_json["Payload"]["Skus"][0]


async def get_uwp_assets(session, data_list):
    """Get all available assets for UWP apps"""
...
<Envelope  xmlns="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://www.w3.org/2005/08/addressing" xmlns:u="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">
    ...
    async with session.post(
        ...
        data=cookie_xml_request,
        headers={"Content-Type": "application/soap+xml; charset=utf-8"},
    ) as response:
        cookie_out = await response.text()

    doc = minidom.parseString(cookie_out)
...
<s:Envelope
...
</s:Envelope>""".format(cookie, cat_id, release_type)

    async with session.post(
        "https://fe3cr.delivery.mp.microsoft.com/ClientWebService/client.asmx",
        data=wuid_xml_request,
        headers={"Content-Type": "application/soap+xml; charset=utf-8"},
    ) as response:
        wuid_out = await response.text()

    doc = minidom.parseString(html.unescape(wuid_out))
    filenames = {}  # {ID: filename}

    # Extract filenames
    for node in doc.getElementsByTagName("Files"):
        try:
            id_node = node.parentNode.parentNode.getElementsByTagName("ID")[0]
            id_value = id_node.firstChild.nodeValue

            installer_id = node.firstChild.attributes['InstallerSpecificIdentifier'].value
            file_name = node.firstChild.attributes['FileName'].value
            modified = node.firstChild.attributes['Modified'].value

            filenames[id_value] = (
                f"{installer_id}_{file_name}",
                modified,
            )
    ...
    if not filenames:
        raise Exception("Server returned empty file list")

    # Extract update identities
    identities = {}  # {filename: (update_id, revision_number)}
    name_modified = {}  # {filename: modified}

    for node in doc.getElementsByTagName("SecuredFragment"):
        try:
            id_node = node.parentNode.parentNode.parentNode.getElementsByTagName("ID")[
                0]
            id_value = id_node.firstChild.nodeValue

            if id_value not in filenames:
                continue

            file_name, modified = filenames[id_value]

            update_identity = node.parentNode.parentNode.firstChild
            update_id = update_identity.attributes["UpdateID"].value
            revision_number = update_identity.attributes["RevisionNumber"].value

            name_modified[file_name] = modified
            identities[file_name] = (update_id, revision_number)
        except (KeyError, AttributeError):
            continue

    if not identities:
        raise Exception("No update identities found")

    # Get download URLs for all assets
    fe3_xml_request = """
<Envelope xmlns="http://www.w3.org/2003/05/soap-envelope"
    ...
    assets = []
    for filename, (update_id, revision_number) in identities.items():
        async with session.post(
            "https://fe3cr.delivery.mp.microsoft.com/ClientWebService/client.asmx/secured",
            data=fe3_xml_request.format(
                update_id, revision_number, release_type),
            headers={"Content-Type": "application/soap+xml; charset=utf-8"},
        ) as response:
            url_out = await response.text()

        doc = minidom.parseString(url_out)
        download_url = None

        for i in doc.getElementsByTagName("FileLocation"):
            try:
                url = i.getElementsByTagName("Url")[0].firstChild.nodeValue
                # Skip blockmap URLs (they have a fixed length of 99 characters)
                if len(url) != 99:
                    download_url = url
                    break
            except (KeyError, AttributeError):
        ...
        if download_url:
            # Parse filename to get architecture and extension
            ...
            if len(parts) >= 3:
                ...
                assets.append({
                    ...
                    "arch": arch,
                    "extension": ext,
                    "modified": name_modified[filename]
                })

    return assets


async def get_non_uwp_assets(session, product_id):
    """Get all available assets for non-UWP apps"""
    ...
    for installer in installer_list:
        download_url = installer.get("InstallerUrl")
        arch = installer.get("Architecture", "unknown").lower()
        locale = installer.get("InstallerLocale", "unknown")
        installer_type = installer.get("InstallerType", "exe")

        if download_url:
            assets.append({
                "name": f"{arch}_{locale}.{installer_type}",
                "url": download_url,
                "arch": arch,
                "extension": installer_type,
                "locale": locale
            })
...
async def download_asset(session, asset, download_dir=None):
...
async def fetch_assets(url):
    """Fetch all available assets for a Microsoft Store URL"""
    ...
    async with aiohttp.ClientSession(
        ...
        data_list = product_info.get("FulfillmentData", None)

        if data_list:
            # UWP app
            assets = await get_uwp_assets(session, data_list)
            is_uwp = True
        else:
            # Non-UWP app
            assets = await get_non_uwp_assets(session, product_id)
            is_uwp = False
